...
$sql = "SELECT lf.leave_id, ud.firstName, ud.lastName, lf.leave_type, 
        lf.start_date, lf.end_date, lf.place, lf.status, lf.dept_head_id, lf.dean_id 
        FROM leave_forms lf
        JOIN user_details ud ON lf.employee_id = ud.ID
        WHERE lf.status = 'Pending'
        AND (lf.dept_head_id = '$user_id' OR lf.dean_id = '$user_id')";
        ...
        <div class="container-fluid page-content mt-3">
          ...
          <div class="main-container">
            ...
            <div class="leave-status-container">
              <a href="leave_pending.php" class="btn leave-status active">Pending</a>
              <a href="leave_approved.php" class="btn leave-status">Approved</a>
              <a href="leave_declined.php" class="btn leave-status">Declined</a>
            </div>
            <div class="table-container">
              <div class="container table-scontainer">
                <h3 class="table-name">Pending Leave Submissions</h3>
                  ...
                  <tbody class="text-center" id="employeeTableBody">
                    ...
                    if (mysqli_num_rows($result) > 0) {
                      ...
                      while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td>";
                        echo "<button class='accept-pending-button' data-leave-id='" . $row['leave_id'] . "'><i class='fas fa-check'></i></button>";
                        echo "<button class='decline-pending-button' data-leave-id='" . $row['leave_id'] . "'><i class='fas fa-xmark'></i></button>";
...
<div class="modal fade" id="declineReasonModal" tabindex="-1" aria-labelledby="declineReasonModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="declineReasonModalLabel">Decline Leave Request</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form id="declineReasonForm">
          <input type="hidden" id="declineLeaveId" name="leave_id">
          <div class="mb-3">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-danger" id="confirmDecline">Decline Leave</button>
      </div>
    </div>
  </div>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.accept-pending-button').forEach(function (button) {
    button.addEventListener('click', function () {
      ...
      processLeaveAction(leaveId, 'approve');
    });
  });
  ...
  document.getElementById('confirmDecline').addEventListener('click', function() {
    const leaveId = document.getElementById('declineLeaveId').value;
    ...
    processLeaveAction(leaveId, 'decline', declineReason);
    declineReasonModal.hide();
  });
});

function processLeaveAction(leaveId, action, declineReason = '') {
  ...
  fetch('actions/process_leave.php', {
    method: 'POST',
