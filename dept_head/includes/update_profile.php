...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($password)) {
        ...
        $sqlDeptHeadCredentials = "UPDATE deptHead_credentials SET password = ? WHERE ID = ?";
        if ($stmt = $conn->prepare($sqlDeptHeadCredentials)) {
    ...
    if ($stmt = $conn->prepare($sqlUserDetails)) {
        $stmt->bind_param("sssssssssissssssssssssss", $firstName, $middleName, $lastName, $department, $jobTitle, $contactNumber, $email, $address, $birthDate, $age, $birthPlace, $sex, $religion, $civilStatus, $nationality, $educationalQualification, $dateHired, $sssNumber, $tinNumber, $pagibigNumber, $philhealthID, $profileImage, $signature, $username);
