...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    switch ($role) {
        case 'deptHead':
            $sql = "UPDATE leave_forms SET dept_head_approval = ?, decline_reason = ? WHERE leave_id = ?";
            break;
        case 'dean':
            $sql = "UPDATE leave_forms SET dean_approval = ?, decline_reason = ? WHERE leave_id = ?";
            break;
        case 'admin':
            $sql = "UPDATE leave_forms SET admin_approval = ?, decline_reason = ? WHERE leave_id = ?";
            break;
        default:
    ...
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ssi", $new_status, $decline_reason, $leave_id);
