...
// Function to check and update Make-up Class status
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    switch ($role) {
        case 'deptHead':
            $sql = "UPDATE make_up_forms SET dept_head_approval = ?, decline_reason = ? WHERE makeup_class_id = ?";
            break;
        case 'dean':
            $sql = "UPDATE make_up_forms SET dean_approval = ?, decline_reason = ? WHERE makeup_class_id = ?";
            break;
        case 'admin':
            $sql = "UPDATE make_up_forms SET admin_approval = ?, decline_reason = ? WHERE makeup_class_id = ?";
            break;
        default:
    ...
    $stmt = mysqli_prepare($conn, $sql);
    mysqli_stmt_bind_param($stmt, "ssi", $new_status, $decline_reason, $makeup_class_id);
