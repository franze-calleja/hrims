...
$sql = "SELECT logf.log_id, ud.firstName, ud.lastName, logf.log_date, 
        logf.log_time, logf.log_in_out, logf.status, logf.decline_reason, logf.dept_head_id, logf.dean_id 
        FROM log_form logf
        JOIN user_details ud ON logf.employee_id = ud.ID
        WHERE logf.status = 'Declined'
        ...
        <div class="container-fluid page-content mt-3">
          ...
          <div class="main-container">
            ...
            <div class="leave-status-container">
              <a href="log_pending.php" class="btn leave-status" >Pending</a>
              <a href="log_approved.php" class="btn leave-status" >Approved</a>
              <a href="log_declined.php" class="btn leave-status active" >Declined</a>
            </div>
            <div class="table-container">
              <div class="container table-scontainer">
                <h3 class="table-name">Declined Log Submissions</h3>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      document.querySelectorAll('.accept-pending-button').forEach(function (button) {
        button.addEventListener('click', function () {
          ...
          processLeaveAction(logId, 'approve');
        });
      });
      ...
      document.querySelectorAll('.decline-pending-button').forEach(function (button) {
        button.addEventListener('click', function () {
          const logId = this.getAttribute('data-log-id');
          processLeaveAction(logId, 'decline');
        });
      });
    });

    function processLeaveAction(logId, action) {
      fetch('actions/process_log.php', {
        method: 'POST',
        headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
        body: new URLSearchParams({ log_id: logId, action: action })
      })
        .then(response => response.json())
        .then(data => {
          if (data.success) {
            alert(data.success);
            location.reload(); // Reload the page to update the leave status in the table
          } else {
            alert(data.error);
          }
        })
