...
<body>
                  ...
                  <tbody id="employeeTableBody">
                    ...
                    if (mysqli_num_rows($result) > 0) {
                      ...
                      while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        <td>
                            <button class='edit-button'><i class='fas fa-upload'></i></button>
                        </td>
                          </div>";
                        echo "</tr>";
                      }
  ...
  <div class="modal fade" id="uploadCertificateModal" tabindex="-1" aria-labelledby="uploadCertificateModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="uploadCertificateModalLabel">Upload Employee Certificate</h5>
          <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
        </div>
        <form id="certificateUploadForm" action="upload_certificate.php" method="POST" enctype="multipart/form-data">
          <div class="modal-body">
            ...
            <div class="mb-3">
              <label for="certificateFile" class="form-label">Upload Certificate</label>
              <div class="dropzone border border-primary rounded p-5 text-center"
                onclick="document.getElementById('certificateFile').click()" ondragover="event.preventDefault()"
                ondrop="handleFileDrop(event)">
                <i class="fas fa-cloud-upload-alt fa-3x text-primary mb-3" style="color: #800000 !important;"></i>
                <p>Drag & drop your file here or click to upload</p>
                <p class="small text-muted">(Accepted formats: .pdf, .jpg, .jpeg, .png)</p>
              </div>
              <input type="file" class="form-control d-none" id="certificateFile" name="certificate_file"
                accept=".pdf,.jpg,.jpeg,.png" required onchange="handleFileChange(event)">
            </div>
          </div>
  ...
  <script>
    function handleFileChange(event) {
      ...
      document.querySelector('.dropzone p').textContent = fileName + " selected";
    }
  ...
  <script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
      // Add click event to upload buttons in the table
      const uploadButtons = document.querySelectorAll('.edit-button');
      uploadButtons.forEach(button => {
        button.addEventListener('click', function () {
          ...
          // Update modal title
          document.getElementById('uploadCertificateModalLabel').textContent =
            `Upload Certificate for ${firstName} ${lastName}`;

          // Show the modal
          var uploadModal = new bootstrap.Modal(document.getElementById('uploadCertificateModal'));
          uploadModal.show();
        });
      });
    });
