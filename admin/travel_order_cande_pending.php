...
$sql = "SELECT toc.travel_order_id, ud.firstName, ud.lastName, toc.destination, 
        ...
        toc.status, toc.dept_head_id, toc.dean_id 
        FROM travel_order_candelaria_forms toc
        JOIN user_details ud ON toc.employee_id = ud.ID
        WHERE toc.status = 'Pending'";
      ...
      <div class="col p-0">
        ...
        <div class="container-fluid page-content mt-3">
          ...
          <div class="main-container">
            ...
            <a href="travel_order_cande_pending.php" class="btn leave-status active">Pending</a>
              <a href="travel_order_cande_approved.php" class="btn leave-status">Approved</a>
              <a href="travel_order_cande_declined.php" class="btn leave-status">Declined</a>
            </div>
            <div class="table-container">
              <div class="container table-scontainer">
                <h3 class="table-name">Pending Travel Order Candelaria Submissions</h3>
                  ...
                  <tbody class="text-center" id="employeeTableBody">
                    ...
                    if (mysqli_num_rows($result) > 0) {
                      ...
                      while ($row = mysqli_fetch_assoc($result)) {
                        ...
                        echo "<td>" . date('h:i A', strtotime($row['travel_time'])) . "</td>";
                        echo "<td>" . date('h:i A', strtotime($row['return_time'])) . "</td>";
                        ...
                        echo "<td>";
                        echo "<button class='accept-pending-button' data-travel-order-cande-id='" . $row['travel_order_id'] . "'><i class='fas fa-check'></i></button>";
                        echo "<button class='decline-pending-button' data-travel-order-cande-id='" . $row['travel_order_id'] . "'><i class='fas fa-xmark'></i></button>";
...
<div class="modal fade" id="declineReasonModal" tabindex="-1" aria-labelledby="declineReasonModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="declineReasonModalLabel">Decline Travel Order Candelaria Request</h5>
        <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
      </div>
      <div class="modal-body">
        <form id="declineReasonForm">
          <input type="hidden" id="declinetravelOrderCId" name="travel_order_id">
          <div class="mb-3">
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  document.querySelectorAll('.accept-pending-button').forEach(function (button) {
    button.addEventListener('click', function () {
      const travelOrderCId = this.getAttribute('data-travel-order-cande-id');
      processTravelOrderCAction(travelOrderCId, 'approve');
    });
  });
  ...
  document.querySelectorAll('.decline-pending-button').forEach(function (button) {
    button.addEventListener('click', function () {
      const travelOrderCId = this.getAttribute('data-travel-order-cande-id');
      // Set the leave ID in the decline modal
      document.getElementById('declinetravelOrderCId').value = travelOrderCId;
  ...
  document.getElementById('confirmDecline').addEventListener('click', function() {
    const travelOrderCId = document.getElementById('declinetravelOrderCId').value;
    ...
    processTravelOrderCAction(travelOrderCId, 'decline', declineReason);
    declineReasonModal.hide();
  });
});

function processTravelOrderCAction(travelOrderCId, action, declineReason = '') {
  const formData = new URLSearchParams();
  formData.append('travel_order_id', travelOrderCId);
  formData.append('action', action);
  if (declineReason) {
    formData.append('decline_reason', declineReason);
  }

  fetch('actions/process_travel_order_cande.php', {
    method: 'POST',
