...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['empID'])) {
    ...
    // Prepare and bind
    $stmt = $conn->prepare("INSERT INTO leave_forms (employee_id, leave_type, dept_head_id, dean_id, start_date, end_date, days, place, reason, status, admin_approval, dept_head_approval, dean_approval, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

    $stmt->bind_param("sssssssssssssss", $empID, $leaveType, $deptHeadReport, $deanReport, $startDate, $endDate, $days, $place, $reason, $status, $adminApproval, $deptHeadApproval, $deanApproval, $createdAt, $updatedAt);
